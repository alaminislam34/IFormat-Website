      ...
      <div className="max-w-360 mx-auto w-11/12">
        <ScrollReveal yOffset={40}>
          <SectionHeader
            title={
              <>
                About <GradientText>iFormat</GradientText> Branding
              </>
            }
            description="At iFormat Branding, we see brand equity as a key asset for individuals and businesses. Whether advancing your career, attracting investors, boosting market presence, or enhancing employer reputation, perception shapes your opportunities. Our approach blends technology and psychology to build visibility, trust, and lasting brand value through strategic positioning, storytelling, ATS optimization, and digital branding."
          />
        </ScrollReveal>
          ...
          {cards.map((card, idx) => (
            <ScrollReveal key={idx} yOffset={40} delay={idx * 0.15}>
              <div className="bg-slate-50 rounded-3xl p-8 text-center hover:shadow-xl transition-shadow border border-slate-100 flex flex-col items-center h-full">
                <div className="w-14 h-14 bg-brand-cyan rounded-xl flex items-center justify-center mb-6 shadow-md shadow-cyan-500/20">
                  {card.icon}
                </div>
